...
// https://sourcegraph.com/github.com/sourcegraph/sourcegraph@7ce0484274d7382f4b9b2898d77c3987e51fffa6/-/blob/client/shared/src/search/query/decoratedToken.ts
...
const decorateContainsBody = (body: string, offset: number): DecoratedToken[] | undefined => {
    ...
    for (const token of result.term) {
        if (token.type === 'filter' && token.field.value === 'file') {
            decorated.push(...decorate(mapOffset(token, offset)))
        }
        ...
        decorated.push(mapOffset(token, offset))
    }
    return decorated
}
